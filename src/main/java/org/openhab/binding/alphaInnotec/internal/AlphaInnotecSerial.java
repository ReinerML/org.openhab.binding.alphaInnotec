...
public class AlphaInnotecSerial implements SerialPortEventListener {
    ...
    private void assembleFrame(byte[] wordIn) {
        ...
        // Sync received?
        if ((wordLast[0] == 0xff) && (wordLast[1] < 3) && (wordLast[0] == wordInLow) && (wordLast[1] == wordInHigh)) {
        ...
        if (wordIndex > 109) {
            if (inSync) {
                ...
                logger.error("SYNC frame missing!");
            }
            wordIndex = 0;
        }
    ...
    private void extractDataFromFrame() {
        ...
        String temp = calculateTemp(frame[AlphaInnotecBindingConstants.INDEX_TEMP_AMBIENT_AIR][1],
                frame[AlphaInnotecBindingConstants.INDEX_TEMP_AMBIENT_AIR + 1][1]);
        if (!temp.equals("")) {
            logger.debug("temperature ambient air: " + temp + " °C");
            parent.publishData(AlphaInnotecBindingConstants.CHANNEL_TEMP_AMBIENT_AIR, new DecimalType(temp));
        }
        ...
        if (!temp.equals("")) {
            logger.debug("temperature inlet air: " + temp + " °C");
            parent.publishData(AlphaInnotecBindingConstants.CHANNEL_TEMP_INLET_AIR, new DecimalType(temp));
        }
        ...
        if (!temp.equals("")) {
            logger.debug("temperature exhaust air: " + temp + " °C");
            parent.publishData(AlphaInnotecBindingConstants.CHANNEL_TEMP_EXHAUST_AIR, new DecimalType(temp));
        }
        ...
        if (!temp.equals("")) {
            logger.debug("temperature heat exchange air: " + temp + " °C");
            parent.publishData(AlphaInnotecBindingConstants.CHANNEL_TEMP_HEAT_EXCHANGE_AIR, new DecimalType(temp));
        }
        ...
        if (!temp.equals("")) {
            logger.debug("temperature cooling plate: " + temp + " °C");
            parent.publishData(AlphaInnotecBindingConstants.CHANNEL_TEMP_COOLING_PLATE, new DecimalType(temp));
        }
        ...
        if (!temp.equals("")) {
            logger.debug("temperature tap water top: " + temp + " °C");
            parent.publishData(AlphaInnotecBindingConstants.CHANNEL_TEMP_TAP_WATER_TOP, new DecimalType(temp));
        }
        ...
        if (!temp.equals("")) {
            logger.debug("temperature tap water bottom: " + temp + " °C");
            parent.publishData(AlphaInnotecBindingConstants.CHANNEL_TEMP_TAP_WATER_BOTTOM, new DecimalType(temp));
        }
        ...
        if (!temp.equals("")) {
            logger.debug("temperature ext. storage unit: " + temp + " °C");
            parent.publishData(AlphaInnotecBindingConstants.CHANNEL_TEMP_EXT_STORAGE_UNIT, new DecimalType(temp));
        }
    ...
    private String calculateTemp(final short tempHigh, final short tempLow) {
        String result = "";
        float temp = tempHigh * 256.0F;
        temp += tempLow;
        temp = (temp / 10) - 30;
        temp = ((int) (temp * 100)) / 100.0F;
    ...
    private void startTimer() {
        ...
        TimerTask task = new TimerTask() {
            ...
            public void run() {
                ...
                inSync = false;
                logger.error(msg);
                parent.updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.COMMUNICATION_ERROR, msg);
                timer.cancel();
            }
        };
        timer = new Timer();
